...
export default defineConfig({
  ...
  manifest: {
    ...
    // Only "sidePanel" is actually exercised by the code. Reading the active tab's
    // URL (for FR-4 button gating) works off host_permissions below, no "tabs"/
    // "activeTab" grant needed; there's no chrome.scripting.executeScript call
    // since the content script auto-injects via the matches below.
